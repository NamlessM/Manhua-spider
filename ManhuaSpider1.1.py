...
def downvideo_for_one_chapter(i):
    ...
    path = str(user_book)+'/'+str(i)
    num = inverse_search[i]
    ...
    if path not in  os.listdir('./'):
        ...
        os.mkdir(path)
            ...
            #url = manhua_chapters[num]
            #print(url)
        x = collect_link('https://'+url)+1
        for i in range(1,x):
            #print(collect_link('https://'+url))
            #get_image_link('https://'+url,i)
            detail_path = path + '/' +str(i)+'.jpg'
            urlretrieve(get_image_link('https://www.'+url,i),detail_path)
...
if __name__ == "__main__":
    ...
    pool = ThreadPool(2)
    results = pool.map(downvideo_for_one_chapter, intended_chpaters)
    pool.close()
    pool.join()
